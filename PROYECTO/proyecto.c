...
#define MINIATRURE_SIZE (15360)         // Tamaño de la miniatura en bytes
...
void lcd_putCustomPhoto( uint8 *photo, uint16 height, uint16 width, uint16 x, uint16 y );
...
void main( void )
{
    ...
    lcd_bmp2photo( ARBOL, album.pack[i].data->photo );
    album.pack[i].secs = 0;
    album.pack[i].effect = efectoNulo;
    album.pack[i].sense = NO_APPLY;
    i++;

    lcd_bmp2photo( PICACHU, album.pack[i].data->photo );
    album.pack[i].secs = 3;
    ...
    while( 1 )    
    {   
        (*album.pack[i].effect)( album.pack[i].data->photo, album.pack[i].sense );    // Ejecuta el efecto para visualizar la nueva foto
        test( album.pack[i].data->photo );                                            // Chequea que el resultado del efecto es el deseado
...
void lcd_bmp2photo( uint8 *bmp, uint8 *photo )
{
    ...
    headerSize = bmp[10] + (bmp[11] << 8) + (bmp[12] << 16) + (bmp[13] << 24);    // Los datos de cabecera est�n en little-endian
...
void lcd_bmp2min( uint8 *bmp, uint8 *min )
{
    ...
    for( ySrc=0, yDst=MIN_HEIGHT-1; ySrc<MIN_HEIGHT; ySrc++, yDst-- )             // Voltea verticalmente e invierte los pixels
    {
        offsetDst = yDst*MIN_WIDTH/2;
        offsetSrc = ySrc*LCD_WIDTH*0.2;
        for( x=0; x<MIN_WIDTH/2; x++ )
            min[offsetDst+x] = ~bmp[offsetSrc + (uint16)(x * 2.5)];
    }
}
...
**
** Deber� recodificarse para que se realice mediante una �nica operaci�n DMA ya que los pixeles de una imagen son contiguos en memoria
*/
...
void lcd_putCustomPhoto( uint8 *photo, uint16 height, uint16 width, uint16 x, uint16 y )
{
    ...
    for(auxY = y; auxY < height; auxY++)
        for(auxX = x; auxX < width; auxX++)
            lcd_buffer[(auxY*LCD_COLS)+auxX] = photo[(auxY*width)+auxX];
}
...
void lcd_shift( uint8 sense )
{
    ...
    switch( sense )
    {
        case LEFT:                                                                              // Al ser un desplazamiento a izquierda, tener en cuenta que columnas consecutivas son contiguas en memoria pero los segmentos de fila no (excepto que sean filas completas y consecutivas)
            ...
            {
                ZDISRC0  = (0 << 30) | (1 << 28) | (uint32) (lcd_buffer + (y*LCD_COLS) + 1);    // datos de 8b, direcci�n POST-INCREMENTADA, origen: posici�n de la segunda columna de la fila correspondiente
                ZDIDES0  = (2 << 30) | (1 << 28) | (uint32) (lcd_buffer + (y*LCD_COLS));        // recomendada, direcci�n POST-INCREMENTADA, destino: posici�n de la primera columna de la fila correspondiente
                ZDICNT0  = (2 << 28) | (1 << 26) | (0 << 22) | (0 << 21) | (LCD_COLS-1);        // whole service, unit tranfer mode, pooling mode, no autoreload, tama�o: el de columnas menos una
