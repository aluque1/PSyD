...
void iic_start( uint8 mode, uint8 byte )
{
    ...
    */
    ...
    IICSTAT = (((mode & 0x3) << 6)| (1 << 5));// Escribimos un 1 en ICCSTAT[5] (esto genera el start condition) y ponemos el modo IICSTAT[7:6] como se pasa por arg
...
uint8 iic_getByte( uint8 ack )
{
    ...
    IICCON &= ~(1 << 4);
    while(IICSTAT & 1); // espera la recepcion de un dato 
    return IICDS;
}
